...
public class CtrPredict {
    ...
    static Logger logger = Logger.getLogger(CtrPredict.class);
    ...
    public static void main(String[] args) {
        ...
        // args = new String[] { "/Users/xmly/deviceId-adItemId-2016011821.show",
        ...
        args = new String[] { "./data/province-ad-2015011821/province-adItemId-2016011821.show",
                ...
                "file:/Users/xmly/git/damus/damus-mining/data/province-ad-2015011821/output" };

        JavaSparkContext sc = new JavaSparkContext("local[2]", "CtrPredict");
        SparkContext rawSc = JavaSparkContext.toSparkContext(sc);

        // 加载点击和展现数据并merge到一起
        JavaPairRDD<Tuple2<Integer, Integer>, CtrDataPoint> showDps = sc.textFile(args[0], 6)
                .mapToPair(new CtrDataPointReader(true));
        JavaPairRDD<Tuple2<Integer, Integer>, CtrDataPoint> clickDps = sc.textFile(args[1], 6)
                .mapToPair(new CtrDataPointReader(false));
        ...
        // 单个预测
        // double predictResult = model.predict(11509, "ffffffff-ffff-0801-0205-376c0037d7ef"
        // .hashCode());
        ...
        JavaPairRDD<Tuple2<Integer, Integer>, Double> checkExpects = checkDps
                .mapToPair(new PairFunction<Tuple2<Tuple2<Integer, Integer>, DataPoint>, Tuple2<Integer, Integer>, Double>() {
                    ...
                    public Tuple2<Tuple2<Integer, Integer>, Double> call(
                            Tuple2<Tuple2<Integer, Integer>, DataPoint> t) throws Exception {
                        return new Tuple2<Tuple2<Integer, Integer>, Double>(t._1(), t._2()
                                .getRating());
                    }
                });
        ...
        // 计算校验集上的均方误差（RMSE）
        JavaRDD<Tuple2<Object, Object>> predictionAndObservations = predictionAndObservationsDouble
                .values().map(new Function<Tuple2<Double, Double>, Tuple2<Object, Object>>() {
                    @Override
                    public Tuple2<Object, Object> call(Tuple2<Double, Double> tuple)
                            throws Exception {
